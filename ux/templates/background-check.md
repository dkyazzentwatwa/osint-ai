...
**Search Queries:**
```
/recon "{{full_name}}" {{location}}
/dork "{{full_name}}" {{location}} (profile OR directory)
/dork "{{full_name}}" intitle:"{{location}}"
...
```
/dork "{{full_name}}" LinkedIn
/dork "{{full_name}}" {{location}} site:linkedin.com/in
/recon "{{full_name}}" --platform linkedin
```

2. **Social Media**
```
/dork "{{full_name}}" (Twitter OR X) {{location}}
/dork "{{full_name}}" Facebook profile
/dork "{{full_name}}" Instagram {{location}}
/dork "@{{full_name}}" - site:twitter.com
```

3. **Specialized Platforms**
```
/dork "{{full_name}}" GitHub OR Stack Overflow
/dork "{{full_name}}" Medium OR Substack
/dork "{{full_name}}" YouTube channel
/dork "{{full_name}}" Pinterest OR TikTok
```

4. **Forum/Community Presence**
```
/dork "{{full_name}}" OR "{{username}}" (forum OR reddit)
/dork "{{full_name}}" site:quora.com
```
...
```
/dork "{{full_name}}" "{{claimed_employer}}"
/dork "{{full_name}}" {{location}} "worked at" OR "employed"
/recon "{{full_name}}" --employment-history
/pivot "{{full_name}}" --colleagues {{claimed_employer}}
```

**Education Verification:**
```
/dork "{{full_name}}" "{{claimed_school}}" alumni OR graduate
/dork "{{full_name}}" site:alumni.{{school_domain}}
/dork "{{full_name}}" "class of" {{graduation_year}}
```

**Professional Credentials:**
```
/dork "{{full_name}}" "{{certification}}" license OR certified
/dork "{{full_name}}" site:{{professional_association}}.org
/recon "{{full_name}}" --certifications
```
...
/recon "{{full_name}}" --comprehensive

# Timeline reconstruction
/timeline "{{full_name}}" --from {{earliest_year}}

# Connection mapping
/pivot "{{full_name}}" --discover-connections

# Content analysis
/dork "{{full_name}}" (blog OR article OR interview)
```
...
|-----------|--------------|-------|
| Profiles | All social accounts | /recon, /dork |
| Content | Posts, articles, comments | /dork, /timeline |
| Photos | Public images | Reverse image search |
| Connections | Associates, colleagues | /pivot |
| Interests | Groups, forums, topics | Content analysis |
| Timeline | Activity over time | /timeline |
...
```
/dork "{{full_name}}" {{location}} court OR lawsuit
/dork "{{full_name}}" arrest OR charged OR indictment
/dork "{{full_name}}" "sex offender" OR "criminal record"
/recon "{{full_name}}" --public-records
```

**News and Media:**
```
/news "{{full_name}}" {{location}} --years 10
/dork "{{full_name}}" {{location}} news OR article
```

**Professional Standing:**
```
/dork "{{full_name}}" "disciplinary action" OR "sanctioned"
/dork "{{full_name}}" "barred" OR "suspended" OR "disbarred"
/recon "{{full_name}}" --professional-standing
```
...
4. Timeline Summary
   ...
   Consistency status: [consistent/gaps found]
...
```
/report generate background-check

Formats available:
  /report export summary     → 1-page overview
  /report export detailed    → Full report (5-10 pages)
  /report export complete    → All raw data included
  /report export timeline    → Chronological view only
  /report export csv         → Spreadsheet format
  /report export pdf         → Professional PDF

Privacy options:
  /report --redact-sensitive → Remove PII for sharing
  /report --legal-review     → Format for legal use
```
